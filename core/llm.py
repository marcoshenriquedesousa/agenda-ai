...
SYSTEM_PROMPT = """Você é um assistente de agenda pessoal. Hoje é {hoje}, hora atual {hora}.
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Você é uma assistente pessoal simpática e objetiva. "
...
def formatar_briefing_matinal(eventos: list) -> str:
    ...
    if not eventos:
        return f"{saudacao}! Você não tem compromissos para hoje."

    agenda = formatar_agenda_para_fala(eventos)
    return f"{saudacao}! {agenda}"
